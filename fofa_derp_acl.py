...
# Probe constants
HTTP_BODY_SAMPLE_SIZE = 256
STUN_MIN_RESPONSE_LEN = 20
MAX_SORT_LATENCY_MS = 999999.0
USER_AGENT = "derp-scout/1.0"
FOFA_API = "https://fofa.info/api/v1/search/all"

# Reusable SSL contexts to avoid repeated initialization
_SSL_DEFAULT_CTX = ssl.create_default_context()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stun-port",
        type=int,
        default=DEFAULT_STUN_PORT,
        help="STUN probe port, default: 3478",
    )
    parser.add_argument(
        "--allow-ip-hostname",
        action="store_true",
...
def normalize_host(raw_host: str, ip: str, port: int, protocol: str) -> Tuple[str, str, int]:
    ...
    # Build final_host: if hostname already has a scheme, use as-is; otherwise prepend protocol:port
    if hostname.startswith(("http://", "https://")):
        final_host = hostname
    elif protocol and protocol.lower() in {"http", "https"}:
...
def fofa_search(email: str, key: str, query: str, page: int, size: int, fields: List[str], full: bool, timeout: float) -> List[Candidate]:
...
def load_candidates_from_file(path: str) -> List[Candidate]:
    with open(path, "r", encoding="utf-8") as fh:
        ...
        if first_line.startswith("["):
            # JSON array — need to read the full file
            fh.seek(0)
...
def tls_probe(hostname: str, port: int, timeout: float) -> Tuple[bool, Optional[float], bool, Optional[str], List[str], Optional[str]]:
    ...
    try:
        with socket.create_connection((hostname, port), timeout=timeout) as sock:
            with _SSL_DEFAULT_CTX.wrap_socket(sock, server_hostname=hostname) as wrapped:
        ...
        try:
            with socket.create_connection((hostname, port), timeout=timeout) as sock:
                with _SSL_UNVERIFIED_CTX.wrap_socket(sock, server_hostname=hostname) as wrapped:
                    cert = wrapped.getpeercert()
            elapsed = (time.perf_counter() - start) * 1000.0
            subject, san = extract_cert_names(cert)
...
def http_probe(hostname: str, port: int, path: str, timeout: float) -> Tuple[bool, Optional[int], Optional[float], bool, Optional[str]]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout, context=_SSL_DEFAULT_CTX) as resp:
            body = resp.read(HTTP_BODY_SAMPLE_SIZE).decode("utf-8", "replace")
        ...
        elapsed = (time.perf_counter() - start) * 1000.0
        body = exc.read(HTTP_BODY_SAMPLE_SIZE).decode("utf-8", "replace")
        hint = "derp" in body.lower() or path == "/derp/probe"
        return True, exc.code, round(elapsed, 2), hint, None
    except urllib.error.URLError as exc:
        return False, None, None, False, f"url error: {exc.reason}"
...
def stun_probe(hostname: str, port: int, timeout: float) -> Tuple[bool, Optional[float], Optional[str]]:
    ...
    try:
        ...
        elapsed = (time.perf_counter() - start) * 1000.0
        if len(data) >= STUN_MIN_RESPONSE_LEN and data[4:8] == bytes.fromhex("2112A442"):
            return True, round(elapsed, 2), None
...
def _probe_http(
    hostname: str, port: int, probe_path: str, fallback_probe_path: str, timeout: float
) -> Tuple[bool, Optional[int], Optional[float], bool, Optional[str]]:
    """Probe HTTPS, trying primary path then fallback."""
    http_ok, http_status, http_ms, http_hint, http_error = http_probe(hostname, port, probe_path, timeout)
    if not http_ok and fallback_probe_path and fallback_probe_path != probe_path:
        http_ok, http_status, http_ms, http_hint, http_error = http_probe(hostname, port, fallback_probe_path, timeout)
    return http_ok, http_status, http_ms, http_hint, http_error


def _probe_stun(hostname: str, port: int, timeout: float) -> Tuple[bool, Optional[float], Optional[str]]:
    return stun_probe(hostname, port, timeout)
...
def probe_candidate(
    ...
    tls_ok, tls_ms, tls_verified, cert_subject, cert_san, tls_error = False, None, False, None, [], None
    http_ok, http_status, http_ms, http_hint, http_error = False, None, None, False, None
    stun_ok, stun_ms, stun_error = False, None, None
    ...
    if tls_ok and tls_verified:
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as exec:
            http_future = exec.submit(_probe_http, candidate.hostname, candidate.port, probe_path, fallback_probe_path, timeout)
            stun_future = exec.submit(_probe_stun, candidate.hostname, stun_port, timeout)
            http_ok, http_status, http_ms, http_hint, http_error = http_future.result()
            stun_ok, stun_ms, stun_error = stun_future.result()

    latency_ms = tls_ms or tcp_ms

    selected, reason = evaluate_candidate_selection(
        candidate=candidate,
    ...
    return ProbeResult(
        ...
        source={
            ...
            "city": candidate.city,
            "as_organization": candidate.as_organization,
...
def sort_key(result: ProbeResult) -> Tuple[int, float, float]:
    ...
    latency = result.tls_ms or result.tcp_ms or MAX_SORT_LATENCY_MS
    return tls_score + http_score, latency, result.stun_ms or MAX_SORT_LATENCY_MS


def make_node_name(region_id: int, index: int) -> str:
    return f"{region_id}a{index}"
...
def main() -> int:
    ...
    for item in candidates:
        host_key = item.hostname or item.ip
        key = (host_key, item.port, item.ip)
        if key not in unique:
            unique[key] = item

    results: List[ProbeResult] = []
    total = len(unique)
    print(f"Probing {total} candidate(s)...", file=sys.stderr)
    completed = 0
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, args.workers)) as pool:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if completed % max(1, total // 10) == 0 or completed == total:
                print(f"\r  {completed}/{total} completed  ", file=sys.stderr, end="", flush=True)
